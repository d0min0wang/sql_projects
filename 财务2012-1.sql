...
or year(t1.FDate)= CAST(@FYear-1 as varchar(4))
...
insert into #JRC
select t1.FSupplyID,t2.FDate,dateadd(m,11,t2.FDate),t1.FDate,t1.FConsignAmount
from #JRTempA t1
...
select FCustID,
	sum(FAmount)
from #JRC where FDate>=FMinDate and year(FDate)= CAST(@FYear-1 as varchar(4))
Group BY FCustID
--今年
INSERT INTO #JRF
select FCustID,
	sum(FAmount)
from #JRC where year(FDate)= CAST(@FYear as varchar(4)) and FDate<=FMaxDate
Group BY FCustID
--
select * from #JRD t1
...
ISNULL(t1.FAmount1,0) AS "去年1月" , 
ISNULL(t1.FAmount2,0) AS "去年2月" , 
ISNULL(t1.FAmount3,0) AS "去年3月" , 
ISNULL(t1.FAmount4,0) AS "去年4月" , 
ISNULL(t1.FAmount5,0) AS "去年5月" , 
ISNULL(t1.FAmount6,0) AS "去年6月" , 
ISNULL(t1.FAmount7,0) AS "去年7月" , 
ISNULL(t1.FAmount8,0) AS "去年8月" , 
ISNULL(t1.FAmount9,0) AS "去年9月" , 
ISNULL(t1.FAmount10,0) AS "去年10月" , 
ISNULL(t1.FAmount11,0) AS "去年11月" , 
ISNULL(t1.FAmount12,0) AS "去年12月" , 
ISNULL(t10.FAmountAll,0) AS "去年合计" , 
ISNULL(t1.FAmount13,0) AS "今年1月" , 
ISNULL(t1.FAmount14,0) AS "今年2月" , 
ISNULL(t1.FAmount15,0) AS "今年3月" , 
ISNULL(t1.FAmount16,0) AS "今年4月" , 
ISNULL(t1.FAmount17,0) AS "今年5月" , 
ISNULL(t1.FAmount18,0) AS "今年6月" , 
ISNULL(t1.FAmount19,0) AS "今年7月" , 
ISNULL(t1.FAmount20,0) AS "今年8月" , 
ISNULL(t1.FAmount21,0) AS "今年9月" , 
ISNULL(t1.FAmount22,0) AS "今年10月" , 
ISNULL(t1.FAmount23,0) AS "今年11月" , 
ISNULL(t1.FAmount24,0) AS "今年12月" , 
ISNULL(t11.FAmountAll,0) AS "今年合计" , 
...
-- =============================================
-- EXECUTE p_xy_SaleByNewCust '2012'
-- =============================================
