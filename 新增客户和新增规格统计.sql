...
select '累计新增客户销售额(元)' AS fname, 
	...
	[销售额]=ISNULL(SUM(CASE WHEN
                                (convert(varchar(10),t3.F_123,120)>='2021-07-01' and convert(varchar(10),t3.F_123,120)<'2022-07-01') and
    ...
    --                            (convert(varchar(10),t3.F_123,120)>='2022-01-01' and convert(varchar(10),t3.F_123,120)<'2022-02-01') and
    --                            (convert(varchar(10),t1.FDate,120)>='2022-01-01' and convert(varchar(10),t1.FDate,120)<'2022-07-01')
    --                              then t2.FConsignAmount END),0)
from ICStockBill t1 
...
SELECT  '累计新增规格销售额(元)' AS fname,
		...
		[销售额]=ISNULL(SUM(CASE WHEN
                                (convert(varchar(10),t2.FCreateDate,120)>='2020-07-01' and convert(varchar(10),t2.FCreateDate,120)<'2021-07-01') and
                                (convert(varchar(10),v1.FDate,120)>='2021-01-01' and convert(varchar(10),v1.FDate,120)<'2021-07-01')
                                  then u1.FConsignAmount END),0)
        --[今年]=ISNULL(SUM(CASE WHEN
        --                        (convert(varchar(10),t2.FCreateDate,120)>='2022-01-01' and convert(varchar(10),t2.FCreateDate,120)<'2022-02-01') and
...
--		sum(u1.FConsignAmount)
    ...
    and YEAR(t2.FCreateDate) IN ('2020' ,'2021')
