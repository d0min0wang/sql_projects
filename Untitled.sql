...
SELECT FDepartment AS 事业部,FBigTrade AS 行业,
    ...
    P_Money_2 AS '2014销售额',
    P_Money_3 AS '2015销售额',
    P_Money_4 AS '2016销售额',
    P_Money_5 AS '2017销售额',
    P_Money_6 AS '2018销售额',
	--P_AuxQty_1 AS '2009出货量',
    P_AuxQty_2 AS '2014出货量',
    P_AuxQty_3 AS '2015出货量',
    P_AuxQty_4 AS '2016出货量',
    P_AuxQty_5 AS '2017出货量',
    P_AuxQty_6 AS '2018出货量'
FROM(
    SELECT FDepartment=CASE WHEN GROUPING(v3.FName)=1 THEN '<销售部合计>' ELSE (v3.FName) END,
        FBigTrade=CASE WHEN GROUPING(v5.FName)=1 THEN '<事业部合计>' ELSE (v5.FName) END,    
		--P_Money=ISNULL(SUM(u1.FConsignAmount),0),
   ...
   --FROM t_xySaleReporttest
    ...
    ORDER BY a.FDepartment,P_Money_6 DESC
--     
--select   RIGHT(CONVERT(char(6),getdate(),112),2)    
--select month(getdate())
...
--		ISNULL(SUM(CASE CONVERT(char(6),v1.FDate,112) WHEN @Period THEN u1.FConsignAmount END),0),
		...
		[2017销售额]=ISNULL(SUM(CASE when year(v1.FDate) = '2017' AND Month(v1.FDate)<='12'  then v1.FConsignAmount END),0),
		[2018销售额]=ISNULL(SUM(CASE when year(v1.FDate) = '2018' AND Month(v1.FDate)<='12'  then v1.FConsignAmount END),0),
		...
		[2017出货量]=ISNULL(SUM(CASE when year(v1.FDate) = '2017' AND Month(v1.FDate)<='12'  then v1.FAuxQty END),0),
		[2018出货量]=ISNULL(SUM(CASE when year(v1.FDate) = '2018' AND Month(v1.FDate)<='12'  then v1.FAuxQty END),0)
--		sum(u1.FConsignAmount)
    --FROM t_xySaleReporttest
    --select v1.FDate,v3.FName,v2.F_110,v2.Fname,u1.FAuxQty,u1.FConsignAmount
    FROM 
	...
	from [AIS20140921170539].[dbo].ICStockBill t1 
	INNER JOIN [AIS20140921170539].[dbo].ICStockBillEntry t2 ON t1.FInterID=t2.FInterID
	where year(t1.FDate)>='2013' and month(t1.FDate)<='12' and 
t1.FTranType=21 
And t1.FCheckerID>0 ) v1
	group by v1.FSupplyID)t1
LEFT JOIN t_Organization v2 ON t1.FSupplyID=v2.FItemID
LEFT JOIN t_Item v3 ON v2.Fdepartment=v3.FItemID
LEFT JOIN t_Item v4 ON v2.F_117=v4.FItemID
