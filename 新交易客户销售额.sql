...
(
	...
	from ICStockBill
	where FDate>='2021-06-01' AND FDate<'2022-07-01'
	AND
...
select tt3.FName AS 部门,
	...
	tt2.F_101 AS 客户分类,
	tt2.F_123 AS 新增日期,
	tt1.FConsignAmount2021 AS [2021年销售额],
	tt1.FConsignAmount2022 AS [2022年销售额]
...
AS
(
	SELECT FItemID,F_123 FROM t_Organization WHERE F_123<'2020-01-01'
),
cte_old0
AS
(
...
AS
(
	SELECT FItemID,F_123 FROM t_Organization WHERE year(F_123) IN ('2020','2021')
),
CTE1
AS
(
...
(
select t1.FSupplyID as FSupplyID,
	[FConsignAmount2020]=ISNULL(SUM(CASE when year(t1.FDate)= '2020' then t2.FConsignAmount END),0),
	[FConsignAmount2021]=ISNULL(SUM(CASE when year(t1.FDate)= '2021' then t2.FConsignAmount END),0)
	--sum(t2.FConsignAmount) AS FConsignAmount,
	--[FAuxQtySum2018]=ISNULL(SUM(CASE when year(t1.FDate)= '2018' then t2.FAuxQty END),0),
	--[FAuxQtySum2019]=ISNULL(SUM(CASE when year(t1.FDate)= '2019' then t2.FAuxQty END),0)
from ICStockBill t1 
INNER JOIN ICStockBillEntry t2 ON t1.FInterID=t2.FInterID
where year(t1.FDate) in('2020','2021')
and
...
select tt3.FName AS 部门,
	...
	tt2.F_101 AS 客户分类,
	tt2.F_123 AS 新增日期,
	tt1.FConsignAmount2020 AS [2020年销售额],
	tt1.FConsignAmount2021 AS [2021年销售额],
	CASE (tt1.FConsignAmount2021-tt1.FConsignAmount2020) when 0 then 0 else (tt1.FConsignAmount2021-tt1.FConsignAmount2020)/tt1.FConsignAmount2020 END AS Increase
	--tt1.FAuxQtySum2018 AS [2018年出货量],
	--tt1.FAuxQtySum2019 AS [2019年出货量]
 from CTE_RESULT tt1
LEFT JOIN t_Organization tt2 ON tt1.FSupplyID=tt2.FItemID
...
LEFT JOIN t_Item tt5 ON tt2.F_117=tt5.FItemID
LEFT JOIN t_Emp tt6 ON tt2.Femployee=tt6.FItemID
where tt1.FConsignAmount2020>=100000
and
(tt1.FConsignAmount2021-tt1.FConsignAmount2020)/tt1.FConsignAmount2020>0.3
...
select tt3.FName AS 部门,
	...
	--tt2.F_101 AS 客户分类,
	--tt2.F_123 AS 新增日期,
	sum(tt1.FConsignAmount2021) AS [2021年销售额],
	sum(tt1.FConsignAmount2022) AS [2022年销售额]
	--tt1.FAuxQtySum2018 AS [2018年出货量],
	--tt1.FAuxQtySum2019 AS [2019年出货量]
 from CTE_RESULT tt1
LEFT JOIN t_Organization tt2 ON tt1.FSupplyID=tt2.FItemID
